...
func (client *Client) initUpload(masterUrl string, filename string, filesize uint64) (*models.UploadInitResponse, error) {
	uploadRequest := models.UploadInitRequest{
		FileName: filename,
		FileSize: filesize,
	}
...
func (client *Client) uploadToChunkservers(uploadInitResponse *models.UploadInitResponse, file *os.File, fileSize int64, identifier string) {
	...
	for i := uint64(0); i < uploadInitResponse.NumberOfChunks; i++ {
		...
		for k := 0; k < len(uploadInitResponse.Chunkservers); k++ {
			randomChunkserver := uploadInitResponse.Chunkservers[k]
...
func (client *Client) initGet() (*models.GetResponse, error) {
